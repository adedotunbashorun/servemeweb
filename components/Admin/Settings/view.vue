<template>
<div>
    <h3 class="page-title">{{ page }}<nuxt-link to="/admin/settings" class="pull pull-right btn btn-outline-primary"><i class="fa fa-arrow-left"></i> Go Back</nuxt-link></h3>
    <div class="row">
      <div class="col-md-6">
        <!-- PANEL HEADLINE -->
        <div class="panel panel-headline">
          <div class="panel-heading">
              ...
              <span class="text-uppercase panel-subtitle">Fill form to update settings</span>
          </div>
          <div class="panel-body">
            <form @submit.prevent="checkForm">
                    ...
                    <div class="row">
                        <div class="col">
                        <button type="submit" class="mb-2 btn btn-primary mr-2">Update Settings</button>
                        </div>
                    </div>
                </form>
          </div>
...
export default {
  ...
  methods: {
    getSettings(){
        this.$store.dispatch('SettingsById', [this.$nuxt._route.params.id,this.$store.state.auth.headers])
        .then((resp) => {
             this.data = resp.data.settings
        }).catch(err => console.log())
    },

    update(){
        let component = this;
        this.$store.dispatch('addSettings', [component.data,this.$store.state.auth.headers])
        .then((resp) => {
          this.error = ''
          this.success = ''
          if(resp.data.error){
